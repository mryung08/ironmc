  ...
  async execute(interaction, client) {
    ...
    if (interaction.customId == "open-ticket") {
      ...
      interaction.guild.channels.create(`ticket-${interaction.user.username}`, {
        ...
        interaction.reply({
          content: `Ticket è stato creato! <#${c.id}>`,
          ephemeral: true
        });
        ...
        const embed = new client.discord.MessageEmbed()
          .setColor('ffffff')
          .setAuthor('Reason', ' ')
          .setDescription('scegli un motivo per cui apri il ticket')
        ...
        const row = new client.discord.MessageActionRow()
          .addComponents(
            ...
            .setPlaceholder('scegli un motivo per cui apri il ticket')
            .addOptions([{
        ...
        collector.on('collect', i => {
          if (i.user.id === interaction.user.id) {
            if (msg.deletable) {
              msg.delete().then(async () => {
                const embed = new client.discord.MessageEmbed()
                  .setColor('ffffff')
                  .setAuthor('Ticket', ' ')
                  .setDescription(`<@!${interaction.user.id}> ha creato un **Ticket** con la ragione ・ ${i.values[0]}`)
                  .setFooter('Ticket System', ' ')
                  .setTimestamp();
            ...
            if (i.values[0] == 'Candidatura Staff') {
              c.edit({
